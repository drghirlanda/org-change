...
a simple "track changes" feature similar to some word processors. The
main use case for authors to highlight the changes between two
versions of a document, such as before and after review by a third
party.
...
can set adding ~#+latex_header:~ lines.
...
When exporting, org-change looks first at the value of the variable
~org-change-final~. This is initially ~nil~, meaning that the export
proceeds according to the selected backend as detailed below. If
~org-change-final~ is not ~nil~, then only the new text is exported,
resulting in a "clean" new document without any markup. To achieve
